...
def validate_domain_config(domain_name, config, schema):
    ...
    is_findings = (
        isinstance(config, dict) and str(config.get("type")).upper() == "FINDINGS"
    )

    if is_general:
        return _validate_general(
            domain_name,
            config,
        ...
        return _validate_findings(
            domain_name,
            config,
            schema["schemas"]["findings_domain"],
            schema["definitions"],
        )
    else:
        ...
        print(
            f"Validation Warning: {domain_name} structure unrecognized (Not List or Findings Dict)."
        )
        return False
...
def _validate_general(domain, config, schema, definitions):
    if not isinstance(config, list):
        print(
            f"Schema Error: {domain} must be a list (General Domain). Got {type(config)}"
        )
        return False
